...
class loader(Dataset): 
  ...
  def __getitem__(self, idx):
    ...
    face_path = os.path.join(self.root, face)
    print(f"Loading image from: {face_path}")  # Print the image path
    fimg = cv2.imread(face_path)

    if fimg is None:  # Check if the image is loaded correctly
        raise ValueError(f"Failed to load image from path: {face_path}")
